//
// Created by qiubing01 on 2022/12/6.
//
...
Java_com_example_nativehook_ThreadHook_enableThreadHookNative(JNIEnv *env, jclass type) {
    enableThreadHook();
}
...
static bool InitJniEnv(JavaVM *vm) {
    ...
    kJavaClass = reinterpret_cast<jclass>(env->NewGlobalRef(
            env->FindClass("com/example/nativehook/ThreadHook")));
    if (kJavaClass == NULL) {
